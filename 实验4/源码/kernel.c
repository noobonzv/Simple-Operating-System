...
void MyCmd(){
	...
	while(1){
		...
		readcmd();
        if(strcmp(recv,help) == 1 ||strcmp(recv,quit) == 1 || strcmp(recv,list) == 1 || strcmp(recv,run) == 1 || strcmp(recv,clear) == 1 || recv[0] == 'r' || strcmp(recv,batch) == 1){
			prints("\r\n");
			if(strcmp(recv,help) == 1){							
				...
				for(i = 1; i < len0; i++){
					...
					if(index_of_prog > 0 && index_of_prog < 6){	/*只有5个程序*/
