...
extern void run_test_of_7(int seg, int offset);
...
void helpList(){
	...
	prints("-t: to run test program for lab7.\n\r");
...
void MyCmd(){
	...
	while(1){
		...
		readcmd();
        if( strcmp(recv,help) == 1 ||strcmp(recv,quit) == 1 || strcmp(recv,list) == 1 || strcmp(recv,clear) == 1 || recv[0] == 'r' || recv[0] == 'p' || strcmp(recv,batch) == 1 || strcmp(recv,shutdown) == 1 || strcmp(recv,test)==1){
			prints("\r\n");
			...
			else if(strcmp(recv,test) == 1){		/* 实验7新增 */
				cls();
				setMyInt();
