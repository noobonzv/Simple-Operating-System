...
void runprog(int index){
 ...
 void load_multi_process(char* cmdp){
	int i = 0;
	int j = 0;
	int num_of_p = 0;				
	int len = strlen(cmdp);
...
void MyCmd(){
	...
	while(1){
		...
		readcmd();
        if( strcmp(recv,help) == 1 ||strcmp(recv,quit) == 1 || strcmp(recv,list) == 1 || strcmp(recv,clear) == 1 || recv[0] == 'r' || recv[0] == 'p' || strcmp(recv,batch) == 1 || strcmp(recv,shutdown) == 1){
			prints("\r\n");
